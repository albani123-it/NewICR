Public Class foundationPoolDet
...
#End Region
    ...
    Sub cekUserAuthentication()
        ...
        Try
            For i As Integer = 0 To dtUser.Rows.Count - 1
                If dtUser.Rows(i).Item("ld_lm_description") = "lvl_bsl_fon_add_pool" Or dtUser.Rows(i).Item("ld_lm_description") = "lvl_bsl_fon_edit_pool" Then
                    tf = True
                End If
            Next
            If Not tf Then
    ...
    Sub isiData(ByVal polid)
        Dim ds As DataSet
        Dim dt As DataTable
        ds = oAsset.getDetailOfPoolPD(polid, "PD")
        dt = ds.Tables(0)
        If dt.Rows.Count > 0 Then
            ...
            txtPDValue.Text = dt.Rows(0).Item("pol_pd")
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If liBodyLoadAction.Text = "" Then
            ...
            If ViewState("polid") IsNot Nothing Then
                Dim ds As DataSet
                Dim dt As DataTable
                ds = oAsset.getDetailOfPoolPD(ViewState("polid"), "PD")
                dt = ds.Tables(0)
                If dt.Rows.Count > 0 Then
                    Try
                        oAsset.updatePoolAsset(oModel, usrLogin, "UPDATE", ViewState("moid"), "PD", ViewState("polid"))
                        Dim logs As String = objLog.getLogString(Request, Session("UserLoginID"), "PD Pooling Updated - NAME=" & txtPoolName.Text)
                        objLog.insertLog("Pool", logs)
                        liBodyLoadAction.Text = "alert('PD Pool Definition Updated');reloadParentWindow();parent.window.close();"
                    Catch ex As Exception
                ...
                Else
                    Try
                        ...
                        Dim logs As String = objLog.getLogString(Request, Session("UserLoginID"), "PD Pooling Creation - NAME=" & txtPoolName.Text)
                        objLog.insertLog("Pool", logs)
                        liBodyLoadAction.Text = "alert('PD Pool Definition Saved');reloadParentWindow();parent.window.close();"
                    Catch ex As Exception
                        Dim msg As String = ex.Message.Replace("'", "\'")
                        liBodyLoadAction.Text = "alert('" & msg & "');"
                    End Try
