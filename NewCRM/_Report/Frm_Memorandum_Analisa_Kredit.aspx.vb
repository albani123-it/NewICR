...
Imports System.Xml
Public Class Frm_Memorandum_Analisa_Kredit
...
#End Region
    ...
    End Sub
     Public Sub Create_Table()
        ...
        dsData = oRptDKI.getMAK_AnalisisKreditDetail(usrLogin, appno)
        Dim dtData As DataTable
        dtData = dsData.Tables(0)
        If dtData.Columns.Count <> 0 Then
            Dim filePath As String = Server.MapPath("~/xml/MAK/MAK_A.xml")
            Dim xmlDoc As New System.Xml.XmlDocument()
            xmlDoc.Load(filePath)
            Dim nodes As XmlNodeList = xmlDoc.DocumentElement.SelectNodes("/Report")
            litMak.Text = oXMLReport.mainLCR(nodes, dtData)
    ...
    Private Sub exportPDF()
        ...
        Dim fileName As String = "Memorandum_Analisa_Kredit"
        Dim html As String = ""
        Dim strMsg As String = ""
        Dim signPdf As String = SignaturePDF("MAK", appno)
        ...
        strMsg = strMsg.Replace("&", "&")
        'strMsg = strMsg.Replace("""", "")
        strMsg = strMsg.Replace("A.", "<br />A.")
        strMsg = strMsg.Replace("B.", "<br />B.")
        strMsg = strMsg.Replace("C.", "<br />C.")
        ...
        'html &= "<tr><td colspan='2'><p align='center style='font-size:14px;font-weight:bold><b>" & lbljudul.Text & "</b></p></td></tr>"
        'html &= "</table>"
        'html &= " <br />"
        'html &= "<table border = '1' style='width:100%;'>"
        'html &= "<tr><td colspan='2'><p style='text-align:justify;font-size:14px'>" & strMsg.ToString & "</p></td></tr>"
        'html &= "</table>"
        html += "<div align='center'>" & lbljudul.Text & "</div>"
        html += strMsg.ToString()
        html += "<br />"
        html += signPdf
    ...
    Sub Create_Signature()
        Dim html As String = ""
        Dim ds As DataSet
        ds = oRptDKI.get_Signature("MAK", appno)
        Dim dt As DataTable
        dt = ds.Tables(0)
    ...
    Sub checkApproval(ByVal appno)
        Dim ds As DataSet
        ds = oRptDKI.getCheckApproval(appno)
