...
#End Region
    ...
    Function enScore(ByVal modelId, ByVal sidID)
        Dim dt As DataTable = gFunction.exec_text("select a.*, " & _
            vbCrLf & "pd_text, pd_field, lgd_fixed, lgd_formula, lgd_field, ead_text, ead_field, maturity_text, maturity_field " & _
            vbCrLf & "from asset_model a " & _
            vbCrLf & "left join asset_model_radio b on a.mod_id = b.mod_id  where a.mod_id = " & modelId)
        Dim modelinfo = dt.Rows(0)
        mTable = "sim_master_asset_" & sidID
    ...
    Function enScoreSummary(ByVal info)
        ...
        If info.Item("mod_approach") = 3 Then
            ...
            joins = " left join " & rTable & "_score_lgd on " & rTable & "." & uniqueField & " = " & rTable & "_score_lgd." & uniqueField & " " & _
                vbCrLf & " left join " & rTable & "_score_ead on " & rTable & "." & uniqueField & " = " & rTable & "_score_ead." & uniqueField & " "
            drop = "drop table " & rTable & "_score_lgd " & _
                vbCrLf & "drop table " & rTable & "_score_ead "
        End If
        aSql += gFunction.returnLogScript(mTable & "_sc", "START")
        aSql += "IF EXISTS ( SELECT name FROM sysobjects WHERE name = '" & mTable & "_sc' ) " & _
            vbCrLf & "DROP TABLE " & mTable & "_sc "
        aSql += "Select " & rTable & ".*, " & _
                vbCrLf & " scc_code_pd, score_pd, " & _
                vbCrLf & field & _
                vbCrLf & " into " & mTable & "_sc " & _
                vbCrLf & " from " & rTable & " " & _
                vbCrLf & " left join " & rTable & "_score_pd on " & rTable & "." & uniqueField & " = " & rTable & "_score_pd." & uniqueField & " " & _
                vbCrLf & joins
    ...
    Function unScorePD(ByVal info)
        ...
        aSql += "IF EXISTS ( SELECT name FROM sysobjects WHERE name = '" & mTable & "_score_pd' ) " & _
                vbCrLf & "DROP TABLE " & mTable & "_score_pd "
        aSql += "SELECT " & uniqueField & ", '' as scc_code_pd, 0 as score_pd INTO " & mTable & "_score_pd " & _
    ...
    Function enScorePD(ByVal info)
        ...
        Try
            aSql += "IF EXISTS ( SELECT name FROM sysobjects WHERE name = '" & tmpTbl & "' ) " & _
            ...
            aSql += "SELECT *, '" & scc & "' AS scc_code, " & scInfo("scc_additive_points") & " AS additive_points " & _
            ...
            aSql += "IF EXISTS ( SELECT name FROM sysobjects WHERE name = '" & mTable & "_score_pd' ) " & _
                vbCrLf & "DROP TABLE " & mTable & "_score_pd "
            aSql += "SELECT " & uniqueField & ", scc_code as scc_code_pd, score as score_pd INTO " & mTable & "_score_pd " & _
    ...
    Function unScoreLGD(ByVal info)
        ...
        aSql += "IF EXISTS ( SELECT name FROM sysobjects WHERE name = '" & mTable & "_score_lgd' ) " & _
                vbCrLf & "DROP TABLE " & mTable & "_score_lgd "
        aSql += "SELECT " & uniqueField & ", '' as scc_code_lgd, 0 as score_lgd INTO " & mTable & "_score_lgd " & _
                vbCrLf & " FROM " & mTable & " " & vbCrLf
    ...
    Function enScoreLGD(ByVal info)
        ...
        aSql += "IF EXISTS ( SELECT name FROM sysobjects WHERE name = '" & mTable & "_score_lgd' ) " & _
            vbCrLf & "DROP TABLE " & mTable & "_score_lgd "
        aSql += "SELECT " & uniqueField & ", scc_code as scc_code_lgd, score as score_lgd INTO " & mTable & "_score_lgd " & _
                vbCrLf & " FROM " & mTable & "_tmpsc " & vbCrLf
        aSql += gFunction.returnLogScript(mTable & "_score_lgd", "END")
        aSql += "drop table " & mTable & "_tmpsc " & vbCrLf
        spName = gFunction.returnSPName(mdl, scc, "LGD")
    ...
    Function unScoreEAD(ByVal info)
        ...
        aSql += "IF EXISTS ( SELECT name FROM sysobjects WHERE name = '" & mTable & "_score_ead' ) " & _
                vbCrLf & "DROP TABLE " & mTable & "_score_ead "
        aSql += "SELECT " & uniqueField & ", '' as scc_code_ead, 0 as score_ead INTO " & mTable & "_score_ead " & _
                vbCrLf & " FROM " & mTable & " " & vbCrLf
    ...
    Function enScoreEAD(ByVal info)
        ...
        aSql += "IF EXISTS ( SELECT name FROM sysobjects WHERE name = '" & mTable & "_score_ead' ) " & _
            vbCrLf & "DROP TABLE " & mTable & "_score_ead "
        aSql += "SELECT " & uniqueField & ", scc_code as scc_code_ead, score as score_ead INTO " & mTable & "_score_ead " & _
                vbCrLf & " FROM " & mTable & "_tmpsc " & vbCrLf
        aSql += gFunction.returnLogScript(mTable & "_score_ead", "END")
        aSql += "drop table " & mTable & "_tmpsc " & vbCrLf
        spName = gFunction.returnSPName(mdl, scc, "EAD")
