...
#End Region
    ...
    Function enPool(ByVal modelId, ByVal sidID)
        ...
        'Return gFunction.returnSimSPName(modelinfo("mod_id"), "pool", "LGD")
        enPoolPD(modelinfo, sidID)
        If modelinfo.Item("mod_approach") = 3 Then
            enPoolLGD(modelinfo, sidID)
            enPoolEAD(modelinfo, sidID)
        Else
            enPoolFIRB(modelinfo, sidID)
        End If
        Return aSql
    End Function

    Function enPoolFIRB(ByVal info, ByVal sidID)
        ...
        spName = gFunction.returnSimSPName(sidID, "pool", "LGD")
        ...
        spName = gFunction.returnSimSPName(sidID, "pool", "EAD")
        'Return aSql
        gFunction.createSP(spName, aSql)
    End Function

    Function enPoolPD(ByVal info, ByVal sidID)
        unCon = ""
        ...
        spName = gFunction.returnSimSPName(sidID, "pool", "PD")
        gFunction.createSP(spName, aSql)
    End Function

    Function enPoolLGD(ByVal info, ByVal sidID)
        unCon = ""
        sql = "select * from asset_model_pool " & _
            vbCrLf & "left join rules on rul_id=pol_rule " & _
    ...
    Function enPoolEAD(ByVal info, ByVal sidID)
        unCon = ""
        sql = "select * from asset_model_pool " & _
            vbCrLf & "left join rules on rul_id=pol_rule " & _
