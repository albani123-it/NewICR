...
#End Region
    ...
    Function enResult(ByVal modelId)
        ...
        If dt.Rows.Count > 0 Then
            If dt.Rows(0).Item("mod_approach") > 1 Then
                ...
                spName = "en_score_" & modelinfo("mod_id")
                ...
                WriteResult(modelinfo)
                'Return aSql
            ElseIf dt.Rows(0).Item("mod_approach") = 1 Then
                Dim modelinfo = dt.Rows(0)
                mTable = "idx_master_asset_" & modelinfo("mod_ast_id")
                resourceTable = "idx_master_asset_" & modelinfo("mod_id")
                WriteResultSA(modelinfo)
            End If
        End If
    End Function

    Function WriteResultSA(ByVal modelinfo)
        ...
        aSql += "Select " & resourceTable & "." & uniqueField & ", " & _
                vbCrLf & " '' scc_code_pd, 0 score_pd , " & _
                vbCrLf & " '' scc_code_lgd, 0 score_lgd , " & _
                vbCrLf & " '' scc_code_ead, 0 score_ead , " & _
        ...
        spName = gFunction.returnSPName(modelinfo("mod_id"), "result", "")
        'Return aSql
        gFunction.createSP(spName, aSql)
    End Function

    Function WriteResult(ByVal modelinfo)
        ...
        aSql += "Select " & uniqueField & ", scc_code_pd, score_pd, scc_code_lgd, score_lgd, scc_code_ead, score_ead, pd_rating, pd, lgd_rating, lgd, ead_rating, ead, maturity,  CREDIT_LINE_AMT, BALANCE_AMT, (capital_requirement * ead_value)capital_charge, " & _
                ...
                vbCrLf & " " & mTable & ".scc_code_pd, " & mTable & ".score_pd , " & _
                vbCrLf & " " & mTable & ".scc_code_lgd, " & mTable & ".score_lgd , " & _
                vbCrLf & " " & mTable & ".scc_code_ead, " & mTable & ".score_ead , " & _
