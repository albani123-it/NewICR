Public Class enRetail
    Dim oGen As New lGeneral
    Dim CEG As New enRatingCalculation

#Region " Global Variable "
    Dim spName = "ce_retail_"
    Dim spNameChar = "ce_retail_char_"
    Dim spNameCar = "ce_retail_car_"
#End Region

    Public Function rebuildEngine()
        Dim dt As New DataTable
        dt = CEG.getSccforRetail()
        Dim dts As New DataTable
        Dim dtr As New DataTable
        For Each dr In dt.Rows
            Dim sql As New StringBuilder
            sql.AppendLine(" ")
            dtr = CEG.getScRating(dr("prd_retail_scc"))
            ...
            dts = CEG.getScChar(dr("prd_retail_scc"))
            ...
            sql.AppendLine(" FROM losnr_total_summary WHERE lam_noApp = @app_no ")
            sql.AppendLine(" )als ")
            'Return sql
            oGen.createSP(spName + dr("prd_retail_scc"), sql.ToString(), "@app_no varchar(50)")
        Next
    ...
    Sub createChar()
        ...
        For Each dr In dt.Rows
            ...
            For Each drs In dts.Rows
                ...
                sql.AppendLine(" SELECT @id,'" & dr("prd_retail_scc") & "',(select lsp_name from losnr_summary_parameter where lsp_field = '" & drs("sch_field") & "'),")
                sql.AppendLine(" (CASE ")
                For i As Integer = 1 To 20
                    Dim str = ""
                    Dim dtTo As String = drs("sch_limit" & i)
                ...
                sql.AppendLine("where lbr_field = '" & drs("sch_field") & "' and lbd_value = (SELECT " & drs("sch_field") & " FROM losnr_total_summary where lam_noApp = @app_no)")
                sql.AppendLine(")")
                'sql.AppendLine(" END  * " & drs("sch_koefisien_score") & ") ," & "(SELECT " & drs("sch_field") & " FROM losnr_total_summary where lst_noApp = @app_no)")
                sql.AppendLine(" FROM losnr_total_summary WHERE lam_noApp = @app_no  ")
                sql.AppendLine("end")
                ...
                sql.AppendLine(" ELSE " & oGen.intFormat(drs("sch_default_score")) & " ")
                sql.AppendLine(" END ) *" & drs("sch_koefisien_score") & ",")
                sql.AppendLine("(SELECT " & drs("sch_field") & " FROM losnr_total_summary WHERE lam_noApp = @app_no)")
                sql.AppendLine(" FROM losnr_total_summary WHERE lam_noApp = @app_no  ")
                sql.AppendLine("end")
            Next
            oGen.createSP(spNameChar + dr("prd_retail_scc"), sql.ToString(), "@app_no varchar(50),@id int")
    ...
    Function createCAR()
        ...
        For Each dr In dt.Rows
            ...
            For Each drs In dts.Rows
                    ...
                    If Len(drs("sch_limit" & i).ToString) > 0 And Len(drs("sch_score" & i).ToString) > 0 Then
                        ...
                        If Not lGeneral.isDBTypeNumeric(CInt(drs("sch_field_type"))) Then
                            ...
                            ElseIf Not dtTo = "" And dtScore <> "0" Then
                                str += "'" & dtTo & "', "
                                attr = dtTo
                                count = "(SELECT CASE WHEN " & drs("sch_field") & "= '" & dtTo & "' THEN 1 ELSE 0 END FROM losnr_total_summary where lam_noApp = @app_no), "
                                cond = "(SELECT CASE WHEN " & drs("sch_field") & "= '" & dtTo & "' THEN 1 ELSE 0 END FROM losnr_total_summary where lam_noApp = @app_no)"
                            End If
                        Else
                            str += "'>" & dtFrom & " AND <= " & dtTo & "', "
                            attr = ">" & dtFrom & " AND <= " & dtTo
                            count = "(SELECT CASE WHEN " & drs("sch_field") & "> " & dtFrom & " AND " & drs("sch_field") & "<=" & dtTo & " THEN 1 ELSE 0 END FROM losnr_total_summary where lam_noApp = @app_no), "
                            cond = "(SELECT CASE WHEN " & drs("sch_field") & "> " & dtFrom & " AND " & drs("sch_field") & "<=" & dtTo & " THEN 1 ELSE 0 END FROM losnr_total_summary where lam_noApp = @app_no)"
                        End If
                        dtFrom = drs("sch_limit" & i)
                        ...
                        sql.AppendLine("END")
                        sql.AppendLine("IF NOT EXISTS (SELECT * FROM history_characteristic where scorecard_code = '" & dr("prd_retail_scc") & "' and characteristic = '" & drs("sch_field") & "'")
                        sql.AppendLine("and attribute = '" & attr & "' and period = '" & year & month & "')")
                        sql.AppendLine("BEGIN")
                        sql.AppendLine("    INSERT INTO history_characteristic values ( '" & dr("prd_retail_scc") & "', '" & drs("sch_field") & "', ")
                        sql.AppendLine(str)
                        sql.AppendLine("END")
                        sql.AppendLine("ELSE")
                        sql.AppendLine("BEGIN")
                        ...
                        sql.AppendLine("        where scorecard_code = '" & dr("prd_retail_scc") & "' and characteristic = '" & drs("sch_field") & "'")
                        sql.AppendLine("        and attribute = '" & attr & "' and period = '" & year & month & "'")
                        sql.AppendLine("    END")
                        sql.AppendLine("END")
                ...
                sql.AppendLine("    BEGIN")
                sql.AppendLine("        INSERT INTO history_characteristic values ( '" & dr("prd_retail_scc") & "', '" & drs("sch_field") & "', 'Default Attribute', ")
            ...
            Next
            oGen.createSP(spNameCar + dr("prd_retail_scc"), sql.ToString(), "@app_no varchar(50)")
