Public Class LKN
    ...
    Protected pagename As String = "Laporan Kunjungan Setempat"
    ...
    Protected Sub delete(ByVal sender As Object, ByVal e As CommandEventArgs)
        ...
        Try
            oapp.deleteLKN(dataItem("lsl_id"))
            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('Data berhasil dihapus.');window.location.href='LKN.aspx?appno=" & appno.Value & "';", True)
        Catch ex As Exception
            Dim msg As String = ex.Message.Replace("'", "")
            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('" & msg & ".');window.location.href='LKN.aspx?appno=" & appno.Value & "';", True)
        End Try
    End Sub
