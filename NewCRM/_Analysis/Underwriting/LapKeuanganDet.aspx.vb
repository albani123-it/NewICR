...
Public Class LapKeuanganDet
    ...
    Sub generateForm(ByVal ds)
        ...
        For index = 0 To dTbl.Rows.Count - 1
            ...
            Else
                ...
                val = dTbl.Rows(index).Item("add_value")   '''edited by harfiq 12 feb
            End If
            If dTbl.Rows(index).Item("viewed") = 1 Then
                ...
                If dTbl.Rows(index).Item("acc_input") = "Y" Then
                    html += "<td width='5%'><input runat='server' class='clsInput' type='text' onkeypress='return isNumberKey(event)' " & classInput & " id='" & dTbl.Rows(index).Item("acc_code") & "' name='" & dTbl.Rows(index).Item("acc_code") & "' value='" & val & "'  onkeyup='" & dTbl.Rows(index).Item("acc_js_formula") & ";  " & dTbl.Rows(index).Item("acc_js_formula_pct") & ";'/></td>"
                Else
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnsave.Click
        'ds = getAppendAccformula(report, sector, code, txtDate.Text)
        'parseData(Request.Form)
        'Response.Redirect("b_Necara.aspx")
        'Try
        '    ds = getAppendAccformula(report, sector, code, ddltanggal.Text)
    ...
    Protected Sub btnview_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnview.Click
        ...
        'Else
        '    liBodyLoadAction.Text = "alert('" & liBodyLoadAction.Text & "');"
        'End If
        Dim today As Date
        Dim tgls As DateTime
        ...
        If ddltanggal.SelectedValue = "New Periode" Then
            ...
            Else
                ...
                tanggal = txtDate.Text
                today = Date.Now()
                tgls = Convert.ToDateTime(Right(tanggal, 4) + "/" + tanggal.Substring(3, 2) + "/" + Left(tanggal, 2))
                If typ = "Proyeksi" Then
                    If tgls <= today Then
                        cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('Tanggal Periode tidak boleh kurang dari hari ini');", True)
                        txtDate.Focus()
                    End If
                    bindata()
                    'genForm(ds)
    ...
    Sub cekInput()
        ...
        lNot &= oGen.checkInputInaHTML("TextBox", "Tanggal", txtDate.Text)
        liBodyLoadAction.Text = lNot
        'If lNot <> "" Then
        '    liBodyLoadAction.Text = "alert('" & lNot & "');"
        'End If
    End Sub


    Sub checkBalance(ByVal report, ByVal sector)
        Dim lNot As String = ""

        Dim balance

        If report = "Neraca" Then
            If sector = "5" Then
                balance = Convert.ToDecimal(Request.Form("X00421").ToString())
                If (balance <> Convert.ToDecimal("0")) Then
                    lNot = "Tidak balance"
                End If

            Else
                balance = Request.Form("X00420").ToString()
                If (balance <> Convert.ToDecimal("0")) Then
                    lNot = "Tidak balance"
                End If
            End If
        ...
        If lNot <> "" Then
            liBodyLoadAction.Text = lNot
            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('" + lNot + ", Laporan neraca harus balance');", True)
        End If
    ...
    Protected Sub SaveData(ByVal sender As Object, ByVal e As CommandEventArgs)
        liBodyLoadAction.Text = ""
        cekInput()
