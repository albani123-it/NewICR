...
Public Class PakReport
    ...
    Private Shared _me As New PakReport()
    ...
    Sub isiData(ByVal appno)
        Dim ds As DataSet
        Dim dSet As DataSet
        ds = oPak.getPakReportList(appno, "Risk")
        dSet = CheckExistFile(ds)
    ...
    Public Shared Function DownloadFile(ByVal param As String) As String
        Dim json As String = ""
        _me.DownloadFiles(param)
        Return (json)
    End Function

    Sub DownloadFiles(ByVal param)
        Dim type As String = "MBK"
        Dim docname As String = type + "_" + param + ".pdf"
        Dim mkFolder As String
        mkFolder = "~/Document Upload/" & param & "/" & type & "/"
    ...
    Function CheckExistFile(ByVal dataset As DataSet)
        Dim type As String
        Dim mkFolder As String
        ...
        If dt1.Rows.Count > 0 Then
            For index = 0 To dt1.Rows.Count - 1
                If dt1.Rows(index).Item("kode") = "FIN" Then
                    type = dt1.Rows(index).Item("kode")
                    mkFolder = "~/Document Upload/" & appno.Value & "/" + type + "/"
                    filename = type + "_" + appno.Value + ".pdf"
                    If (System.IO.File.Exists(Server.MapPath(mkFolder) & filename)) = False Then
                        dt1.Rows(index)("flag") = "N"
                        dt1.Rows(index)("infoReport") = "Belum ada data"
                    End If
                ElseIf dt1.Rows(index).Item("kode") = "MAK" Then
                    type = dt1.Rows(index).Item("kode")
                    mkFolder = "~/Document Upload/" & appno.Value & "/" + type + "/"
                    filename = type + "_" + appno.Value + ".pdf"
                    If (System.IO.File.Exists(Server.MapPath(mkFolder) & filename)) = False Then
                        ...
                        dt1.Rows(index)("infoReport") = "Belum ada data"
                    End If
                ElseIf dt1.Rows(index).Item("kode") = "IPK" Then
                    type = dt1.Rows(index).Item("kode")
                    mkFolder = "~/Document Upload/" & appno.Value & "/" + type + "/"
                    filename = type + "_" + appno.Value + ".pdf"
                    If (System.IO.File.Exists(Server.MapPath(mkFolder) & filename)) = False Then
