...
Public Class DataAgunanDet
    ...
    End Sub
    Sub checkDataBorroweType(ByVal appno)
        Dim ds As DataSet
        Dim dt As DataTable
        ds = oAgu.getDataBorrowerType(appno)
        dt = ds.Tables(0)
        If (dt.Rows(0)(5).ToString() = "5") Then
            Session("brwtype") = dt.Rows(0)(5).ToString()
            formatMultiFinance()
        Else
            Session("brwtype") = dt.Rows(0)(5).ToString()
            formatOthers()
    ...
    Sub formatMultiFinance()
        rowDtl.Visible = False
        rowStsKepemilikan.Visible = False
        rowBuktiKemungkinan.Visible = False
        rowLokasiAgunan.Visible = False
        rowNamaPemilik.Visible = False
        rowAppraisal.Visible = False
        rowNamaAppraisal.Visible = False
        rowTglAppraisal.Visible = False
        rowNilaiPasar.Visible = False
        rowNilaiLikuidasi.Visible = False
        rowAsuransiPenjamin.Visible = False
        rowNilaiAsuransi.Visible = False
        rowHeaderFotoAgunan.Visible = False
        rowTblFotoAgunan.Visible = False
        rowBtnAddNew.Visible = False
        lblNilaiPengikatan.Text = "Nilai Pengikatan (Maks) %"
        lblNilaiPengikatan.Attributes.Add("style", "font-weight:bold")
    End Sub

    Sub formatOthers()
        rowDtl.Visible = True
        rowStsKepemilikan.Visible = True
        rowBuktiKemungkinan.Visible = True
        rowLokasiAgunan.Visible = True
        rowNamaPemilik.Visible = True
        rowAppraisal.Visible = True
        rowNamaAppraisal.Visible = True
        rowTglAppraisal.Visible = True
        rowNilaiPasar.Visible = True
        rowNilaiLikuidasi.Visible = True
        rowAsuransiPenjamin.Visible = True
        rowNilaiAsuransi.Visible = True
        rowHeaderFotoAgunan.Visible = True
        rowTblFotoAgunan.Visible = True
        rowBtnAddNew.Visible = True
        lblNilaiPengikatan.Text = "Nilai Pengikatan"
        lblNilaiPengikatan.Attributes.Add("style", "font-weight:bold")
    End Sub
    ...
    Sub isiData(ByVal id)
        ...
        If dt.Rows.Count > 0 Then
            ...
            txtNilaiPasar.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_pasar"))
            txtNilaiLikuidasi.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_likuidasi"))
            txtNilaiPengikatan.Text = oGen.moneyFormatLabel(dt.Rows(0).Item("ldan_nilai_pengikatan"))
            txtAsuransiPenjamin.Text = oGen.CheckNullString(dt.Rows(0).Item("ldan_asuransi_penjamin"))
            txtNilaiAsuransi.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_asuransi"))
            ...
            txtLuasTanah.Text = oGen.moneyNumericLabel(dt.Rows(0).Item("luas_tanah"))
            txtLuasBangunan.Text = oGen.moneyNumericLabel(dt.Rows(0).Item("luas_bangunan"))
            txtNilaiPasarLT.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_pasar_lt"))
            txtNilaiPasarLB.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_pasar_lb"))
            txtNilaiLikuidasiLT.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_likuidasi_lt"))
            txtNilaiLikuidasiLB.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_likuidasi_lb"))
            txtNilaiPengikatanLT.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_pengikatan_lt"))
            txtNilaiPengikatanLB.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_pengikatan_lb"))
            ...
            txtNilaiAsuransiLT.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_asuransi_lt"))
            txtNilaiAsuransiLB.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("ldan_nilai_asuransi_lb"))
    ...
    Protected Sub DdlAgunanOnSelectedIndexChanged(sender As Object, e As EventArgs)
        Dim xType As String
        Dim xJenis As String
        Dim dt As DataTable
        xType = ddlTipeAgunan.SelectedValue.ToString() '
    ...
    Protected Sub ChckedChanged(sender As Object, e As EventArgs)
