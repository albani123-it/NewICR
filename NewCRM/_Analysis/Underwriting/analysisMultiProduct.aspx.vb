Public Class analysisMultiProduct
...
#End Region
    ...
    Sub isiData(ByVal no)
        ...
        ds1 = oApp.checkMultiProduct(no)
        dt1 = ds1.Tables(0)

        If dt1.Rows(0).Item("brw_sec") = "1" Then
            'Construction
            ds = oApp.checkWorksheet(no)
            dt = ds.Tables(0)
            If dt.Rows.Count > 0 Then
                ...
                textSGA.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("lws_sga"))
            ...
            End If
        ElseIf dt1.Rows(0).Item("brw_sec") = "3" Or dt1.Rows(0).Item("brw_sec") = "5" Then
            'Manufacturer
            txtCOGS.Text = oGen.moneyFormatTextBox(dTbl.Rows(0).Item("X00098"))
            txtSGA.Text = oGen.moneyFormatTextBox(dTbl.Rows(0).Item("X00076"))
            txtSales.Text = oGen.moneyFormatTextBox(dTbl.Rows(0).Item("X00100"))
            txtARTO.Text = oGen.moneyFormatTextBox(dTbl.Rows(0).Item("X00115"))
            txtITO.Text = oGen.moneyFormatTextBox(dTbl.Rows(0).Item("X00114"))
            txtAPTO.Text = oGen.moneyFormatTextBox(dTbl.Rows(0).Item("X00116"))
            txtTC.Text = oGen.moneyFormatTextBox(dTbl.Rows(0).Item("X00117"))
            txtProyeksiKebutuhanKMK.Text = oGen.moneyFormatTextBox(dTbl.Rows(0).Item("proyeksi"))
            ...
            If dt.Rows.Count > 0 Then
                ...
                txtPembiayaanBankKMK.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("lws_pembiayaan"))
            End If
        ElseIf dt1.Rows(0).Item("brw_sec") = "4" Then
            'MultiFinance
            ds = oApp.checkWorksheet(no)
            dt = ds.Tables(0)
            If dt.Rows.Count > 0 Then
                txtTargetBookingToPenjualan.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("lws_targetBookingPerSales"))
                txtRealisasiBookingToPenjualan.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("lws_realisasiBookingPerSales"))
                txtKelonggaranTarikBankLain.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("lws_kelonggaranTarikBankLain"))
                txtRencanaObligasi.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("lws_rencanaObligasi"))
                txtTotal.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("lws_maxKebutuhanKMK"))
                txtPembiayaanBank.Text = oGen.moneyFormatTextBox(dt.Rows(0).Item("lws_pembiayaan"))
            End If
        End If
    End Sub
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        ds = oApp.checkMultiProduct(appno.Value)
        dt = ds.Tables(0)
        If lblNotif.Text = "" Then
            If appno.Value <> "" Then
                Try
                    ...
                    If dt.Rows(0).Item("brw_sec") = "1" Then
                        'Construction
                        ds = oApp.checkWorksheet(appno.Value)
                        dt = ds.Tables(0)
                        If dt.Rows.Count > 0 Then
                            'update data
                            oApp.updateWorksheetConstructionMP(Request, appno.Value, usrLogin)
                        Else
                            'insert data
                            oApp.insertWorksheetConstructionMP(Request, appno.Value, usrLogin)
                        End If
                    ElseIf dt.Rows(0).Item("brw_sec") = "3" Or dt.Rows(0).Item("brw_sec") = "5" Then
                        'Manufacturer
                        ds = oApp.checkWorksheet(appno.Value)
                        dt = ds.Tables(0)
                        If dt.Rows.Count > 0 Then
                        ...
                        Else
                            ...
                            oApp.insertWorksheetManufacturer(Request, appno.Value, usrLogin)
                        End If
                    ElseIf dt.Rows(0).Item("brw_sec") = "4" Then
                        'MultiFinance
                        ds = oApp.checkWorksheet(appno.Value)
                        dt = ds.Tables(0)
                        If dt.Rows.Count > 0 Then
                            'update data
                            oApp.updateWorksheetMultifinance(Request, appno.Value, usrLogin)
                        Else
                            'insert data
                            oApp.insertWorksheetMultifinance(Request, appno.Value, usrLogin)
                        End If
                    End If
    ...
    Sub CheckSector(ByVal appno)
        Dim ds As DataSet
        Dim dt As DataTable
        ds = oApp.checkMultiProduct(appno)
        dt = ds.Tables(0)
        If dt.Rows(0).Item("brw_sec") = "1" Then
            divConstruction.Visible = True
        ElseIf dt.Rows(0).Item("brw_sec") = "3" Then
            divManufacturer.Visible = True
        ElseIf dt.Rows(0).Item("brw_sec") = "4" Then
            divMultiFinance.Visible = True
        ElseIf dt.Rows(0).Item("brw_sec") = "5" Then
            divManufacturer.Visible = True
        End If
