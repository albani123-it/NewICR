...
Public Class CreditAnalyze
    ...
    Protected pagename As String = "Memorandum Analisa Kredit"
...
#End Region
    ...
    Sub controlRO()
        ...
        txtKelemahanDanRisiko.Enabled = False
        txtAnalisaKebutuhanKredit.Enabled = False
        txtEvaluasiJaminan.Enabled = False
    ...
    Sub isiData(ByVal no, ByVal brw)
        ...
        If dt.Rows.Count > 0 Then
            ...
            txtKelemahanDanRisiko.Text = oGen.CheckNullString(dt.Rows(0).Item("lmk_kelemahanRisiko"))
            txtAnalisaKebutuhanKredit.Text = oGen.CheckNullString(dt.Rows(0).Item("lmk_analisaKredit"))
            txtEvaluasiJaminan.Text = oGen.CheckNullString(dt.Rows(0).Item("lmk_evaluasiJaminan"))
        ...
        If lblNotif.Text = "" Then
            If appno.Value <> "" Then
                Try
                    Dim ds As DataSet
                    Dim dt As DataTable
                    ds = oApp.checkMemoAnalyze(appno.Value)
                    dt = ds.Tables(0)
                    If dt.Rows.Count > 0 Then
                        'update data
                        oApp.updateMemoAnalyze(Request, appno.Value, usrLogin)
                    Else
                        'insert data
                        oApp.insertMemoAnalyze(Request, appno.Value, usrLogin)
                    End If
