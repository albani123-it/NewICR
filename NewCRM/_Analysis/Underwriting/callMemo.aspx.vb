Public Class callMemo
    ...
    Protected pagename As String = "Call Memo"
...
#End Region
    ...
    Protected Sub delete(ByVal sender As Object, ByVal e As CommandEventArgs)
        ...
        Try
            oApp.deleteCallMemo(dataItem("lcm_id"), usrLogin)
            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('" + oGen.NotifikasiBerhasil() + "');window.location.href='callMemo.aspx?appno=" & appno.Value & "';", True)
        Catch ex As Exception
            Dim msg As String = ex.Message.Replace("'", "")
            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('" & msg & ".');window.location.href='callMemo.aspx?appno=" & appno.Value & "';", True)
        End Try
    End Sub
End Class