Public Class CorporateShareholders
    ...
    Protected pagename As String = "Pemegang Saham Badan"
...
#End Region
    ...
    Sub cekApp(ByVal no)
        ...
        If dtbl.Rows.Count > 0 Then
            ...
            Else
                ...
                ds = oApp.checkTotalIndividualShareholders(dtbl.Rows(0).Item("app_brw_code"))
                dt = ds.Tables(0)
                If dt.Rows.Count > 0 Then
                    lblNotif.Text = "Jumlah Pemegang Saham Perorangan : " & dt.Rows(0).Item("lis_kepemilikanSaham") & " % <br/>"
                    divnotif.Attributes.Add("style", "display:''")

                    If CDbl(dt.Rows(0).Item("lis_kepemilikanSaham")) >= 100 Then
                ...
                Dim ds2 As DataSet
                Dim dt2 As DataTable
                ds2 = oApp.checkTotalCorporateShareholders(dtbl.Rows(0).Item("app_brw_code"))
                dt2 = ds2.Tables(0)
                If dt2.Rows.Count > 0 Then
                    lblNotif.Text &= "Jumlah Pemegang Saham Badan : " & dt2.Rows(0).Item("lcs_kepemilikanSaham") & " %"
                    divnotif.Attributes.Add("style", "display:''")
                    If CDbl(dt2.Rows(0).Item("lcs_kepemilikanSaham") + dt.Rows(0).Item("lis_kepemilikanSaham")) >= 100 Then
                        grid1.AllowAddingRecords = False
                    End If
                    sisa.Value = CDbl(dt2.Rows(0).Item("lcs_kepemilikanSaham") + dt.Rows(0).Item("lis_kepemilikanSaham"))
                End If
    ...
    Sub isiData(ByVal brwcode)
        Dim ds As DataSet
        ds = oApp.checkCorporateShareholders(brwcode)
    ...
    Protected Sub delete(ByVal sender As Object, ByVal e As CommandEventArgs)
        ...
        Dim ds As DataSet = oApp.getDetailOfCorporateShareholders(dataItem("lcs_id"))
        Dim dr As DataRow = ds.Tables(0).Rows(0)

        Try
            oApp.deleteCorporateShareholders(dataItem("lcs_id"))
            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('" + oGen.NotifikasiHapus() + "');window.location.href='CorporateShareholders.aspx?appno=" & appno.Value & "';", True)
        Catch ex As Exception
            Dim msg As String = ex.Message.Replace("'", "")
            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('" & msg & ".');window.location.href='CorporateShareholders.aspx?appno=" & appno.Value & "';", True)
        End Try
    End Sub
End Class