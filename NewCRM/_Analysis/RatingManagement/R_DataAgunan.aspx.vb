Public Class R_DataAgunan
    ...
    Protected pagename As String = "Data Agunan"
...
#End Region
    ...
    Sub isiData(ByVal no)
        Dim ds As DataSet
        ds = oAgu.getListDataAgunan(no)

        GridAgunan.DataSource = ds
        GridAgunan.DataBind()
    End Sub

    Protected Sub delete(ByVal sender As Object, ByVal e As CommandEventArgs)
        Dim rowIndex As Integer = Integer.Parse(e.CommandArgument.ToString())
        Dim dataItem As Hashtable = TryCast(GridAgunan.Rows(rowIndex).ToHashtable(), Hashtable)

        Try
            oAgu.deleteDataAgunanByID(dataItem("lda_id"))
            'oRat.deleteFacilityAgunan(dataItem("lda_id"))

            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('" + oGen.NotifikasiBerhasil() + "');window.location.href='DataAgunan.aspx?appno=" & appno.Value & "';", True)
        Catch ex As Exception
            Dim msg As String = ex.Message.Replace("'", "")
            cs.RegisterStartupScript(Page.[GetType](), "alert", "alert('" & msg & ".');window.location.href='DataAgunan.aspx?appno=" & appno.Value & "';", True)
        End Try
    End Sub

    Sub controlRO()
        GridAgunan.AllowAddingRecords = False
        status.Value = 0
        allowDelete = False
    End Sub
