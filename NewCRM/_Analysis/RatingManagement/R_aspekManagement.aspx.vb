Public Class R_aspekManagement
    ...
    Protected pagename As String = "Aspek Management"
...
#End Region
    ...
    Sub isiData(ByVal brwcode)
        Dim ds As DataSet
        Dim dt As DataTable
        ds = oApp.checkAspek(brwcode, "Aspek Management")
        dt = ds.Tables(0)
        If dt.Rows.Count > 0 Then
            exists = True
        End If
    ...
    Function fillAspek()
        ...
        If dt.Rows.Count > 0 Then
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                dsP = oApp.getBankRatingSector(sector)
                dtP = dsP.Tables(0)
                    ...
                    If dt.Rows(i).Item("lbr_field") = dtP.Rows(tmp).Item("lbp_lbr_field") = True Then
                        ...
                        For j As Integer = 0 To dt2.Rows.Count - 1
                            If brw_code.Value <> "" Then
                                ...
                                ds3 = oApp.getDetailOfBorrowerAspek(brw_code.Value, dt2.Rows(j).Item("lbd_parent"))
                                dt3 = ds3.Tables(0)
                                If dt3.Rows.Count > 0 Then
                                    'selected the choice with data for the answer
                                    If dt2.Rows(j).Item("lbd_value") = dt3.Rows(0).Item("lag_value") Then
                                        html &= "<option value='" & dt2.Rows(j).Item("lbd_value") & "' selected>" & dt2.Rows(j).Item("lbd_value") & "</option>"
                                    Else
                                        html &= "<option value='" & dt2.Rows(j).Item("lbd_value") & "'>" & dt2.Rows(j).Item("lbd_value") & "</option>"
                                    End If
                        ...
                        ds5 = oApp.getDetailOfBorrowerAspek(brw_code.Value, dt.Rows(i).Item("lbr_id"))
                        dt5 = ds5.Tables(0)
                        If dt5.Rows.Count > 0 Then
                            html &= "<input type='hidden' id='hid_" & dt.Rows(i).Item("lbr_id") & "' value='" & dt5.Rows(0).Item("lag_value") & "'/>"
                            html &= "</td></td>"
                            ...
                            If read = True Then
                                html &= "<textarea id='txtDesc_" & dt.Rows(i).Item("lbr_id") & "' name='txtDesc_" & dt.Rows(i).Item("lbr_id") & "' class='inpTxtDisabled' readonly='true' rows='3' style='float:none;width:500px;'>" & dt5.Rows(0).Item("lag_desc") & "</textarea>"
                            Else
                                html &= "<textarea id='txtDesc_" & dt.Rows(i).Item("lbr_id") & "' name='txtDesc_" & dt.Rows(i).Item("lbr_id") & "' class='inpTxt' rows='3' style='float:none;width:500px;'>" & dt5.Rows(0).Item("lag_desc") & "</textarea>"
                            End If
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If brw_code.Value <> "" And lblNotif.Text = "" Then
            Try
                ds = oApp.getAspekManagement
                dt = ds.Tables(0)
                For i As Integer = 0 To dt.Rows.Count - 1
                    ...
                    For tmp As Integer = 0 To dtP.Rows.Count - 1
                        If dt.Rows(i).Item("lbr_field") = dtP.Rows(tmp).Item("lbp_lbr_field") = True Then
                            ...
                            oApp.insertUpdateAspek(brw_code.Value, dt.Rows(i).Item("lbr_id"), value, "Aspek Management", desc, usrLogin)
                        End If
                    Next
